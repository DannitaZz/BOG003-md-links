...
const mdLinks = (path_, options) => {
  //let results = [];
  return new Promise(resolve => {
    if (path.isAbsolute(path_) !== true) {
      path_ = path.resolve(path_);
      console.log("La ruta es relativa");
    }
    const theLinks = recurRead(path_)
    .then(array => extractLinks(array))
    .then(urls => validateLinks(urls.slice(40,50), options))
    .then(links => links);
    console.log('Ahora es promesa');
    resolve(theLinks);
  })  
};
